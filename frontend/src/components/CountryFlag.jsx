...
const CountryFlag = ({ countryList, setCountryList, isToggle, setToggle }) => {
	...
	useEffect(() => {
		...
		for (const key of Object.keys(countryDictionary)) {
			const source = `http://purecatamphetamine.github.io/country-flag-icons/3x2/${key}.svg`;
			copy.push(
				<div onClick={(e) => handleSelected(e, key)} className='custom-option' key={key}>
					<img className='flag-icon' src={source} />
					<span className='ms-1'>+{countryDictionary[key]}</span>
				</div>
			);
		}
		setCountryList(copy);
